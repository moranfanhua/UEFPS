...
优菈、李织烟和阿斯卡纶按 `3` 收枪后进入拳击近战状态，左键以约 145 cm 的近战扫掠挥拳并造成 70 点伤害，不消耗弹药。第一人称中右拳会从画面下方推进到准心下缘后收回，左拳留在下方防守，确保主视角能清楚看到出拳过程。优菈和李织烟复用各自骨架的 `Female_Punch` 动作，阿斯卡纶使用适配其骨架的程序化直拳；按 `1` 可恢复步枪。
