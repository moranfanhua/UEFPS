    ...
    for(int32 Operator=0;Operator<4;++Operator)
    {
        ...
        for(int32 Weapon=0;Weapon<Breach::WeaponCount;++Weapon)
        {
            ...
            Add(.15f,[=]()
            {
                ...
                Check(Player()->UsesAK()==(Operator!=1 && Weapon==0) && Player()->UsesM4()==(Operator!=1 && Weapon==1) &&
                    Player()->UsesMP5()==(Operator!=1 && Weapon==2) && Player()->UsesAA12()==(Operator!=1 && Weapon==3),
                    TEXT("All four gun cards equip their adapted guns; Acheron keeps its sword"));
